...
- 装饰器
---
装饰器我认为算是 python 的中高难度的知识了。

我在最开始学 python 的时候，看过相关的教程，但是，当时是云里雾里。而等我，真正做过相关的项目，再来看这个知识点的时候，有一种豁然开朗的感觉。

所以，学习只是学理论是远远不行的，高屋建瓴全是浮华，一步一个脚印才是王道。
...
不难发现，函数 f 被with_logging取代了，当然它的docstring，\_\_name\_\_就是变成了with_logging函数的信息了。好在我们有functools.wraps，wraps本身也是一个装饰器，它能把原函数的元信息拷贝到装饰器里面的 func 函数中，这使得装饰器里面的 func 函数也有和原函数 foo 一样的元信息了。
